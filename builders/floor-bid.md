...
every submission carries a `cancellable` flag (URL query parameter on V1, request field on V3). non-cancellable submissions (`cancellable=false`) are commitments: by sending one, you are telling the relay this bid stands and you will not pull it back to a lower value.
...
- `cancellable=false` and `value <= floor` → the bid is silently dropped. it is not stored, not simulated, and cannot win.
- `cancellable=true` and `value < floor` → if you have a previous bid at-or-above the floor, that previous bid is cancelled (deleted). the new bid itself is not stored as a candidate.
